...
use CodeConfig\IDB\Dropbox\Http\Clients\DropboxHttpClientInterface;
...
class DropboxClient
{
     ...
     *
     * @var \CodeConfig\IDB\Dropbox\Http\Clients\DropboxHttpClientInterface
     */
     ...
     * Get the HTTP Client
     *
     * @return \CodeConfig\IDB\Dropbox\Http\Clients\DropboxHttpClientInterface $httpClient
     */
     ...
     * Set the HTTP Client
     *
     * @param \CodeConfig\IDB\Dropbox\Http\Clients\DropboxHttpClientInterface $httpClient
     *
     ...
     */
    public function sendRequest($request, $response = null)
    {
        //Method
     ...
     *
     * @param \CodeConfig\IDB\Dropbox\DropboxRequest $request
     *
    ...
    protected function prepareRequest(DropboxRequest $request)
    {
        ...
        if ($request->getEndpointType() === 'content' && $request->getEndpoint() !== '/files/get_thumbnail_batch') {
            ...
            } elseif ($request->hasRawData()) {
                //Content Type
                $request->setContentType("application/octet-stream");

                //Request Body (Raw Data)
                $requestBody = $request->getRawData();
        ...
        $dropbox_api_path_root = apply_filters('ccpidb_set_dropbox_api_path_root', '');

        if (!empty($dropbox_api_path_root)) {
            $request->setHeaders(['Dropbox-API-Path-Root' => json_encode([".tag" => "root", "root" => $dropbox_api_path_root])]);
        }
